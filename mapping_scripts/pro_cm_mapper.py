...
try:
    ...
    ###################################################################################################################################
        ...
        # spark = SparkSession.builder.master("spark://master:7077").appName("procedures_mapper").getOrCreate()
        spark = cf.get_spark_session("pro_cm_mapper")

        unmapped_pro_cm = spark.read.option("inferSchema", "false").load(formatted_data_folder_path+"formatted_pro_cm.csv",format="csv", sep="\t", inferSchema="false", header="true",  quote= '"')
    ...
    ###################################################################################################################################
        mapping_pro_cm_type_dict = create_map([lit(x) for x in chain(*partner_dictionaries.pro_cm_pro_cm_type_dict.items())])
    ...
    ###################################################################################################################################
        ...
        pro_cm_with_additional_fileds = cf.append_additional_fields(
            mapped_df = pro_cm,
            file_name = "formatted_pro_cm.csv",
